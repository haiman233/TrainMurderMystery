...
public abstract class DoorBlockEntity extends SyncingBlockEntity {
    ...
    public static <T extends DoorBlockEntity> void clientTick(Level Level, BlockPos pos, BlockState state, T entity) {
        entity.age++;
    }

    public static <T extends DoorBlockEntity> void serverTick(Level Level, BlockPos pos, BlockState state, T entity) {
        if (state.get(DoorPartBlock.OPEN) && !entity.isBlasted()) {
            ...
            if (entity.getCloseCountdown() <= 0) {
                SmallDoorBlock.toggleDoor(state, Level, (SmallDoorBlockEntity) entity, pos);
            }
        } else {
    ...
    public void toggle(boolean silent) {
        if (this.Level == null || this.Level.getTime() == this.lastUpdate || this.isBlasted()) {
            return;
        }
        this.toggleOpen();
        if (!silent) {
    ...
    protected void toggleOpen() {
        if (this.Level != null) {
            this.lastUpdate = this.Level.getTime();
            this.open = !this.open;
            this.Level.addSyncedBlockEvent(this.pos, this.getCachedState().getBlock(), 1, this.open ? 1 : 0);
    ...
    protected void playToggleSound() {
        ...
        this.Level.playSound(null, this.pos, TMMSounds.BLOCK_DOOR_TOGGLE, SoundCategory.BLOCKS, 1f, 1f);
    }
