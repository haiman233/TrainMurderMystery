    ...
    protected ActionResult onUse(BlockState state, Level Level, BlockPos pos, Player player, BlockHitResult hit) {
        if (Level.getBlockEntity(pos) instanceof HornBlockEntity hornBlockEntity) {
            if (Level instanceof ServerWorld serverWorld) {
                ...
                Vec3 mid = Vec3.ofCenter(pos);
                Level.playSound(null, mid.getX(), mid.getY(), mid.getZ(), SoundEvents.BLOCK_CHAIN_BREAK, SoundCategory.BLOCKS, 0.5f, .8f + (Level.random.nextFloat() - .5f) * .2f);
                if (isSoundReady || isOp)
                    Level.playSound(null, mid.getX(), mid.getY() + 3, mid.getZ(), TMMSounds.AMBIENT_TRAIN_HORN, SoundCategory.AMBIENT, 100.0f, 1.0f);
