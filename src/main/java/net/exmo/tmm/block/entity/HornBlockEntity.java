package net.exmo.tmm.block.entity;
...
public class HornBlockEntity extends BlockEntity {
    ...
    public static void tick(Level Level, BlockPos pos, BlockState state, @NotNull HornBlockEntity horn) {
        horn.prevPull = horn.pull;
        if (horn.pull > 0) {
            horn.pull -= .05f;
            if (horn.pull < 0.01f) {
                horn.pull = 0;
                if (Level != null) Level.updateListeners(pos, state, state, Block.NOTIFY_LISTENERS);
            }
        }
        horn.cooldown--;
    }
    ...
    public void pull(int pull) {
        ...
        this.pull = pull;
        if (this.Level != null)
            this.Level.updateListeners(this.pos, this.getCachedState(), this.getCachedState(), Block.NOTIFY_LISTENERS);
        this.markDirty();
    }
