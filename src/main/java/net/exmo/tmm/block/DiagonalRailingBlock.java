...
public class DiagonalRailingBlock extends AbstractRailingBlock {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        BlockState stateForStairsBelow = this.getPlacementState(state, Level.getBlockState(pos.down()));
        if (stateForStairsBelow != null) {
            ...
            return this.tryConnecting(stateForStairsBelow, Level.getBlockState(neighborPos));
        }
        Direction left = facing.rotateYCounterclockwise();
        state = state.with(SHAPE, RailingShape.BOTTOM);
        BlockState stateForStairsLeft = this.getPlacementState(state, Level.getBlockState(pos.offset(left)));
        if (stateForStairsLeft != null) {
            return stateForStairsLeft;
        }
        Direction right = facing.rotateYClockwise();
        return this.getPlacementState(state, Level.getBlockState(pos.offset(right)));
    }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess Level, BlockPos pos, BlockPos neighborPos) {
        BlockState blockState = super.getStateForNeighborUpdate(state, direction, neighborState, Level, pos, neighborPos);
        if (blockState == null) {
            return null;
    ...
    public VoxelShape getCollisionShape(BlockState state, BlockView Level, BlockPos pos, ShapeContext context) {
        return this.getOutlineShape(state, Level, pos, context);
    }
