...
public interface PrivacyBlock {
    ...
    default void toggle(BlockState state, Level Level, BlockPos pos) {
        boolean opaque = !state.get(OPAQUE);
        if (state.get(INTERACTION_COOLDOWN)) {
            Level.setBlockState(pos, state.with(INTERACTION_COOLDOWN, false));
            return;
        } else {
            Level.playSound(null, pos, TMMSounds.BLOCK_PRIVACY_PANEL_TOGGLE, SoundCategory.BLOCKS, 0.1f, opaque ? 1.0f : 1.2f);
        }

        Level.setBlockState(pos, state.with(OPAQUE, opaque).with(INTERACTION_COOLDOWN, true));
        Level.scheduleBlockTick(pos, state.getBlock(), COOLDOWN);
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (this.canToggle(sideState) && sideState.get(OPAQUE) != opaque) {
                ...
                Level.scheduleBlockTick(sidePos, sideState.getBlock(), DELAY);
            }
        }
        for (Direction[] diagonal : DIAGONALS) {
            ...
            if (this.canToggle(diagonalState) && diagonalState.get(OPAQUE) != opaque) {
                Level.scheduleBlockTick(diagonalPos, diagonalState.getBlock(), DELAY);
            }
        }
    }
    ...
    default boolean canInteract(BlockState state, BlockPos pos, Level Level, Player player, Hand hand) {
