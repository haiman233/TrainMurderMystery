...
public class SmallDoorBlock extends DoorPartBlock {
    ...
    public void onPlaced(Level Level, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        Level.setBlockState(pos.up(), state.with(HALF, DoubleBlockHalf.UPPER));
    }
    ...
    public @Nullable BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return pos.getY() < Level.getTopY() - 1 && Level.getBlockState(pos.up()).canReplace(ctx) ? placementState : null;
    }
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(Level Level, BlockState state, BlockEntityType<T> type) {
        return state.get(HALF) == DoubleBlockHalf.LOWER ? super.getTicker(Level, state, type) : null;
    }
                ...
                if (entity.isOpen()) {
                    ...
                    if (player.getMainHandStack().isOf(TMMItems.KEY) || hasLockpick) {
                        ...
                        if (isRightKey || hasLockpick) {
                            if (isRightKey)
                                Level.playSound(null, lowerPos.getX() + .5f, lowerPos.getY() + 1, lowerPos.getZ() + .5f, TMMSounds.ITEM_KEY_DOOR, SoundCategory.BLOCKS, 1f, 1f);
                            if (hasLockpick)
    ...
    static @NotNull ActionResult open(BlockState state, Level Level, SmallDoorBlockEntity entity, BlockPos lowerPos) {
        if (Level.isClient) return ActionResult.SUCCESS;
        toggleDoor(state, Level, entity, lowerPos);
        return ActionResult.CONSUME;
    }

    public static void toggleDoor(BlockState state, Level Level, SmallDoorBlockEntity entity, BlockPos lowerPos) {
        entity.toggle(false);
        ...
        if (neighborState.isOf(state.getBlock())
                ...
                && Level.getBlockEntity(neighborPos) instanceof SmallDoorBlockEntity neighborEntity) {
