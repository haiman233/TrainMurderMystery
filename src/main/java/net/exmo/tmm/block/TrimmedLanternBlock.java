...
public class TrimmedLanternBlock extends ToggleableFacingLightBlock {
    ...
    @Override
    protected boolean onSyncedBlockEvent(BlockState state, Level Level, BlockPos pos, int type, int data) {
        super.onSyncedBlockEvent(state, Level, pos, type, data);
    ...
    @Nullable
    @Override
    protected NamedScreenHandlerFactory createScreenHandlerFactory(BlockState state, Level Level, BlockPos pos) {
        BlockEntity blockEntity = Level.getBlockEntity(pos);
