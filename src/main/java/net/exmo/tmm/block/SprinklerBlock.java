...
public class SprinklerBlock extends WallMountedBlock implements BlockEntityProvider {
    ...
    @Override
    public void neighborUpdate(BlockState state, Level Level, BlockPos pos, Block block, BlockPos fromPos, boolean notify) {
        if (!Level.isClient) {
            if (Level.isReceivingRedstonePower(pos) || Level.isReceivingRedstonePower(fromPos)) {
    ...
    public void scheduledTick(BlockState state, ServerWorld Level, BlockPos pos, Random random) {
        ...
        Level.setBlockState(pos, cycle, Block.NOTIFY_LISTENERS);
        Level.getBlockEntity(pos, TMMBlockEntities.SPRINKLER).ifPresent(entity -> {
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(Level Level, BlockState state, BlockEntityType<T> type) {
        if (!Level.isClient || !type.equals(TMMBlockEntities.SPRINKLER)) {
            return null;
        }
