...
public class LedgeBlock extends HorizontalFacingBlock {
    ...
    public VoxelShape getCollisionShapeBig(BlockState state, BlockView Level, BlockPos pos, ShapeContext context) {
        return switch (state.get(FACING)) {
    ...
    public VoxelShape getCollisionShapeSmall(BlockState state, BlockView Level, BlockPos pos, ShapeContext context) {
        return switch (state.get(FACING)) {
            case NORTH -> NORTH_SHAPE_SMALL;
            case SOUTH -> SOUTH_SHAPE_SMALL;
            case WEST -> WEST_SHAPE_SMALL;
    ...
    protected VoxelShape getCollisionShape(BlockState state, BlockView Level, BlockPos pos, ShapeContext context) {
        if (!context.isAbove(state.getOutlineShape(Level, pos), pos, true)) {
            return getCollisionShapeSmall(state, Level, pos, context);
        }
        return getCollisionShapeBig(state, Level, pos, context);
    }
