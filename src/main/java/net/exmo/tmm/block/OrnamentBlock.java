...
public class OrnamentBlock extends FacingBlock {
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, Level Level, BlockPos pos, Player player, Hand hand, BlockHitResult hit) {
        if (!player.shouldCancelInteraction() && !player.getMainHandStack().isOf(this.asItem())) {
            ...
            BlockState blockBehindOrnament = Level.getBlockState(behindBlockPos);
            return ((AbstractBlockInvoker) blockBehindOrnament.getBlock()).tmm$invokeOnUseWithItem(stack, blockBehindOrnament, Level, behindBlockPos, player, hand, hit.withBlockPos(behindBlockPos));
        }
    ...
    protected ActionResult onUse(BlockState state, Level Level, BlockPos pos, Player player, BlockHitResult hit) {
        if (!player.shouldCancelInteraction() && !player.getMainHandStack().isOf(this.asItem())) {
            ...
            BlockPos behindBlockPos = pos.subtract(new Vec3i(dir.getOffsetX(), dir.getOffsetY(), dir.getOffsetZ()));
            BlockState blockBehindOrnament = Level.getBlockState(behindBlockPos);
            return ((AbstractBlockInvoker) blockBehindOrnament.getBlock()).tmm$invokeOnUse(blockBehindOrnament, Level, behindBlockPos, player, hit.withBlockPos(behindBlockPos));
        }
